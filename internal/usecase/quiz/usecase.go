...
func New(quizClient QuizGRPCClient, questionClient QuestionGRPCClient, resultClient QuizResultGRPCClient) *UseCase {
	return &UseCase{
...
func (uc *UseCase) GetMyQuizzes(ctx context.Context, limit, offset *int32) (*model.GetAllQuizzesResponse, error) {
	return uc.quizClient.GetMyQuizzes(ctx, limit, offset)
}
...
func (uc *UseCase) TakeQuiz(ctx context.Context, id int64) (*model.TakeQuizResponse, error) {
	...
	for i, q := range questionsResp.Questions {
		answers := make([]model.TakeQuizAnswer, len(q.Answers))
